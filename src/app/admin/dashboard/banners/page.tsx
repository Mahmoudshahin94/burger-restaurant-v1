  ...
  return (
    <AdminLayout title="Banners">
      <div className="max-w-4xl">
        ...
        <div className="mb-5 bg-amber-50 border border-amber-200 rounded-2xl px-4 py-3 flex items-start gap-3">
          ...
          <div className="text-sm text-amber-800">
            ...
            <p className="text-amber-700 text-xs leading-relaxed">
              Active banners appear as a full-width carousel on the public menu, replacing the logo/brand hero.
        ...
        {isLoading ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {[...Array(3)].map((_, i) => (
              <div key={i} className="bg-white rounded-2xl h-52 skeleton" />
            ))}
          </div>
        ) : banners.length === 0 ? (
          <div className="bg-white rounded-2xl p-14 text-center text-gray-400 border border-gray-100">
            ...
            <button
              onClick={() => setShowAdd(true)}
              className="mt-5 bg-brand-red hover:bg-brand-red-dark text-white font-semibold px-5 py-2 rounded-xl text-sm transition-colors"
            >
              Add Banner
            </button>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {banners.map((banner) => (
              <BannerCard
                ...
                onToggle={() => handleToggleActive(banner)}
              />
            ))}
          </div>
        )}
      ...
      <Modal
        ...
        title={`Edit Banner — ${editBanner?.title_en || "(no title)"}`}
        size="lg"
      >
        {editBanner && (
          <BannerForm
  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-100 overflow-hidden shadow-sm hover:shadow-md transition-shadow">
      ...
      <div className="relative h-36 bg-gradient-to-br from-gray-100 to-gray-200">
        ...
        <button
          onClick={onToggle}
          className={`absolute top-2 end-2 text-xs font-semibold px-2.5 py-1 rounded-full transition-colors shadow-sm ${
            banner.active
      ...
      <div className="p-4">
        <div className="min-h-[40px]">
          ...
          {banner.title_ar && (
            <p className="text-gray-500 text-xs truncate mt-0.5" dir="rtl">{banner.title_ar}</p>
          )}
        </div>
        {banner.subtitle_en && (
          <p className="text-gray-400 text-xs truncate mt-1">{banner.subtitle_en}</p>
        )}
        {banner.link && (
          <p className="text-blue-400 text-xs truncate mt-1">
            <span className="text-gray-400">Link: </span>{banner.link}
          </p>
        )}
        ...
        <div className="flex items-center gap-2 mt-3 pt-3 border-t border-gray-100">
          <button
            onClick={onEdit}
            className="flex-1 inline-flex items-center justify-center gap-1.5 text-xs font-semibold px-3 py-1.5 rounded-lg bg-blue-50 text-blue-600 hover:bg-blue-100 transition-colors"
          >
          ...
          <button
            ...
            className="flex-1 inline-flex items-center justify-center gap-1.5 text-xs font-semibold px-3 py-1.5 rounded-lg bg-red-50 text-red-500 hover:bg-red-100 transition-colors disabled:opacity-40"
          >
